...
## psychopy data.staircase source code.
## multistair early termination fix
    # (line 1843)     self.currentStaircase = self.thisPassRemaining[0]
    # self.thisPassRemaining[] returns x staircases based on self.method  (for interleaved staircases)
    # example:  in a 4 condition interleaved staircase...
        # initial calls to self.thisPassRemaining, returns 4 individual staircases  
        # [n0,n1,n2,n3] for referring to each condition

        # if method=fullRandom, any n condition may repeat up to 4 times...

        # when any individual condition/stair completes  (meets nReversal & nTrial criteria)
        # AND repeat stairs are in queue in self.thisPassRemaining...
        # MULTISTAIRS terminates early...

        ## TESTING
        #1. is this due to subsequent self._nextIntensity call?
        #2. 
...
from psychopy.data.base import _BaseTrialHandler, _ComparisonMixin
from psychopy.data.utils import _getExcelCellName
...
print('YOU"VE PULLED IN THE CLAB PSYCHOPY STAIRCASE FIX')
...
class StairHandler(_BaseTrialHandler):
    ...
    *nUp* and *nDown* are always considered as 1 until the first reversal
    ...
    def saveAsExcel(self, fileName, sheetName='data',
        ...
        # Write average of last N-2 reversals
        avgLastN_2=np.mean(self.reversalIntensities[2:])
        ws.cell(column=1, row=revN+3,
                value=u"{}".format(avgLastN_2))
        ws.cell(column=2, row=revN+3,
                value=u"{}".format(len(self.reversalIntensities[2:]) ) )
...
class MultiStairHandler(_BaseTrialHandler):
    ...
    def __init__(self, stairType='simple', method='random',
        ...
        ## testing out multistair fix...
        '''
        if self.thisPassRemaining[0] not in self.runningStaircases:
            self.thisPassRemaining.pop(0)
        '''
    ...
    def __next__(self):
        ...
        ##
        ##
        # wu - __next__ is called at the beginning of each trial
        # thisPassRemaining contains [n0,n1,n2,n3] conditions based on method (random)
        # thisPassRemaining[0] gets passed into currentStaircase
        # remove extra conditions if [0] is done. 
        '''
        for ind,item in reversed(list(enumerate(self.thisPassRemaining[1:]))):
            if item.finished:
                print('*** cleaning thisPassRemaining ***')
                del self.thisPassRemaining[ind+1]
        '''
        for ind,item in reversed(list(enumerate(self.thisPassRemaining))):
            if item.finished:
                print('*** cleaning thisPassRemaining ***')
                del self.thisPassRemaining[ind]
        ...
        if (not hasattr(self, 'thisPassRemaining') or
            ...
            if self.runningStaircases:
                ...
                ##
                ##
                ## testing #1, is this cause of early multistair termination??
                print('\n!\n!\n!\n!!! EARLY TERMINATION loc#1!!!')
        ...
        if not self.finished:
            ...
            ##
            ##
            ## testing #2, is this cause of early multistair termination??
            print('\n!\n!\n!\n!!! EARLY TERMINATION loc#2!!!')
    ...
    def _startNewPass(self):
        ...
        # wu - this gets called once every n trials...
        # where n is # of interleaved stairs. 
    ...
    def addResponse(self, result, intensity=None):
        ...
        if self.currentStaircase.finished:
            print('am i really removing anything?')
